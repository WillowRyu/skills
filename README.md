...
Delegates **code implementation, test authoring/execution, and build/lint/type checks by default** — full features and complex work — to a **configured Command Code CLI worker** (`cmd -p`), while the **main model owns requirements, planning, design decisions, work breakdown, and truly final independent verification**. The main model scopes complex work into worker tasks and may read code and run targeted decisive checks for final acceptance, but does not routinely duplicate the worker's full suite. The worker implements, reviews its own diff, runs the relevant checks, fixes defects, re-runs them, and only then hands back a final result; follow-up fixes and retests outside the micro-edit exception below go back to the worker.

- **Role split:** the main model fixes the purpose, requirements, design, allowed change scope, acceptance criteria, and test/verification strategy, and splits complex work into scoped worker tasks; the worker implements and self-verifies; the main model then independently reviews the final diff. When the worker hits material design ambiguity it escalates with evidence and options instead of deciding for itself, and the main model decides so the worker can continue. Do not let the worker self-approve and stop there; send follow-up fixes and retests outside the micro-edit exception back to the worker. Your explicit instructions can always override this split.
- **Micro-edit exception:** the main model may handle a change directly only when it is a clear micro-edit — a single-file local change (or semantics-identical mechanical repetition) with the edit location and expected diff already fixed so no exploration, diagnosis, or design judgment is needed; no new behavior, business logic, or test design; not an API, schema, DB, migration, auth, security, permission, payment, medical-data, infra, concurrency, async, state, cache, dependency, or generated-code change; and no conflict risk with parallel or user edits. Line count is only a secondary indicator, not a qualification or automatic allowance, and any uncertainty, scope growth, or second logical change means delegate. See the [skill](plugins/command-code-delegate/skills/command-code-delegate/SKILL.md) for the full criteria.
...
- **Making delegation your default:** a single delegation does not change how the agent behaves on later tasks. Persistent default delegation requires you to instruct it separately (for example, *"Delegate all implementation and testing to Command Code by default and reserve yourself for planning and review"*).
- **Run logs and sessions:** each worker run streams its NDJSON event output and stderr to a per-run log so the full stream never floods the main context. Afterward the main model reads the final `result` record (`subtype`, `sessionId`, `stopReason`, `usage`, `durationMs`, `finalText`) and exit code, then records the CLI-returned `sessionId` and log path — it never assumes an ID before the CLI returns one. Follow-up fixes for the same feature resume that session explicitly (`cmd -p --resume <sessionId>`) while its context is still valid; a substantially changed design or stale context starts a new session with a concise current handoff. A missing `usage` field is reported as unknown, and no token savings are claimed without a comparison.
- **Worker handoff:** the worker's final report gives its status, changed files, the actual verification commands with exit codes and result summaries, evidence log paths, and unresolved items. Costs, timing, and rework are reported only when measured.
