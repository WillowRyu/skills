...
Delegates **code implementation, test authoring/execution, and build/lint/type checks by default** — full features and complex work, not just simple implementations, repetitive edits, or mechanical transformations — to a **configured Command Code CLI worker** (`cmd -p`), while the **main model owns requirements, planning, design decisions, work breakdown, and truly final independent verification**. The main model scopes complex work into worker tasks and may read code and run targeted decisive checks for final acceptance, but does not routinely duplicate the worker's full suite or fix code itself. The worker implements, reviews its own diff, runs the relevant checks, fixes defects, re-runs them, and only then hands back a final result; every follow-up fix and retest goes back to the worker.

- **Role split:** the main model fixes the purpose, requirements, design, allowed change scope, acceptance criteria, and test/verification strategy, and splits complex work into scoped worker tasks; the worker implements and self-verifies; the main model then independently reviews the final diff. When the worker hits material design ambiguity it escalates with evidence and options instead of deciding for itself, and the main model decides so the worker can continue. Do not let the worker self-approve and stop there, and do not have the main model fix code itself — send all follow-up fixes and retests back to the worker. Your explicit instructions can always override this split.
