...
**코드 구현과 테스트 작성·실행, 빌드·린트·타입 검사**를 기본적으로(단순 구현·반복 수정·기계적 변환뿐 아니라 완전한 기능과 복잡한 작업까지) **설정된 Command Code CLI 작업자**(`cmd -p`)에게 위임하고, **메인 모델은 요구사항·기획·설계 판단·작업 분할·최종 독립 검증을 담당**합니다. 메인 모델은 복잡한 작업을 범위가 명확한 작업 단위로 나눠 위임하고, 최종 수락을 위해 코드를 열람하거나 결정적 확인 실행을 직접 할 수 있지만 작업자의 전체 검사 세트를 상시 중복하거나 코드를 직접 고치지는 않습니다. 작업자는 구현 → 자체 diff 검토 → 관련 검증 실행 → 결함 수정 → 재검증을 마친 뒤에야 최종 결과를 인계하고, 모든 후속 수정·재검증은 다시 작업자에게 맡깁니다.

- **역할 분담:** 메인 모델이 목적·요구사항·설계·수정 허용 범위·완료 기준·검증·테스트 전략을 정하고 복잡한 작업을 범위가 명확한 작업 단위로 나누며, 작업자가 구현과 자체 검증을 수행하고, 다시 메인 모델이 최종 diff를 독립 리뷰합니다. 작업자가 재량으로 해석할 수 없는 중요한 설계 모호성을 만나면 스스로 결정하지 않고 근거와 선택지를 제시해 에스컬레이션하고, 메인 모델이 결정하면 그에 따라 계속 진행합니다. 작업자의 자체 검증만으로 끝내지 말고, 메인 모델이 코드를 직접 고치지도 마세요 — 후속 수정과 재검증은 모두 작업자에게 되돌립니다. 사용자가 명시적으로 지시하면 이 분담을 언제든 덮어쓸 수 있습니다.
