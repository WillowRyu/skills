...
**코드 구현과 테스트 작성·실행, 빌드·린트·타입 검사**를 기본적으로(완전한 기능과 복잡한 작업까지) **설정된 Command Code CLI 작업자**(`cmd -p`)에게 위임하고, **메인 모델은 요구사항·기획·설계 판단·작업 분할·최종 독립 검증을 담당**합니다. 메인 모델은 복잡한 작업을 범위가 명확한 작업 단위로 나눠 위임하고, 최종 수락을 위해 코드를 열람하거나 결정적 확인 실행을 직접 할 수 있지만 작업자의 전체 검사 세트를 상시 중복하지는 않습니다. 작업자는 구현 → 자체 diff 검토 → 관련 검증 실행 → 결함 수정 → 재검증을 마친 뒤에야 최종 결과를 인계하고, 아래 micro-edit 예외를 벗어난 후속 수정·재검증은 다시 작업자에게 맡깁니다.

- **역할 분담:** 메인 모델이 목적·요구사항·설계·수정 허용 범위·완료 기준·검증·테스트 전략을 정하고 복잡한 작업을 범위가 명확한 작업 단위로 나누며, 작업자가 구현과 자체 검증을 수행하고, 다시 메인 모델이 최종 diff를 독립 리뷰합니다. 작업자가 재량으로 해석할 수 없는 중요한 설계 모호성을 만나면 스스로 결정하지 않고 근거와 선택지를 제시해 에스컬레이션하고, 메인 모델이 결정하면 그에 따라 계속 진행합니다. 작업자의 자체 검증만으로 끝내지 말고, micro-edit 예외를 벗어난 후속 수정과 재검증은 작업자에게 되돌립니다. 사용자가 명시적으로 지시하면 이 분담을 언제든 덮어쓸 수 있습니다.
- **micro-edit 예외:** 메인 모델이 직접 처리할 수 있는 것은 명백한 micro-edit뿐입니다 — 단일 파일의 국소 변경(또는 의미가 동일한 기계적 반복 변경)이고 수정 위치와 기대 diff가 이미 확정되어 탐색·진단·설계 판단이 필요 없으며, 새로운 동작·비즈니스 로직·테스트 설계가 없고, API·스키마·DB·마이그레이션·인증·보안·권한·결제·의료데이터·인프라·동시성·비동기·상태·캐시·의존성·생성 코드 변경이 아니며, 병렬 편집·사용자 변경과 충돌할 위험이 없어야 합니다. 변경 줄 수는 보조 지표일 뿐 자격 조건이나 자동 허용 근거가 아니며, 불확실하거나 범위가 커지거나 두 번째 논리 변경이 드러나면 위임합니다. [전체 기준은 스킬](plugins/command-code-delegate/skills/command-code-delegate/SKILL.md)을 참고하세요.
- **실행 설정:** 작업자는 `cmd -p --output-format json`에 작업에 맞는 `--max-turns`를 붙여 비대화형 실행합니다. 모델·추론 강도·권한 모드는 이번 요청 → 적용되는 개인·프로젝트 지침 → 기존 Command Code 설정 순으로 정합니다. 모델이나 추론 강도를 지정하지 않으면 `--model`/`--effort`를 생략해 CLI 기본값을 따르며, 값을 임의로 만들거나 지속 설정을 조용히 바꾸지 않습니다. 권한 모드를 강제하지 않고 CLI 설정을 따르며, `--yolo`는 사용자가 이미 명시적으로 허용한 경우에만 추가합니다.
- **사용자 설정:** 명시하지 않은 항목은 Command Code 설정과 CLI 기본값을 따르므로, 개인 기본값(직접 지침에 두는 특정 모델·추론 강도 등)은 스킬에 고정하지 않고 그 실행에서 존중됩니다.
- **위임을 기본값으로 만들기:** 한 번의 위임이 이후 작업의 에이전트 동작을 바꾸지는 않습니다. 지속적인 기본 위임은 사용자가 별도로 지시해야 합니다 (예: *"모든 구현과 테스트는 기본적으로 Command Code에 위임하고, 너는 기획과 리뷰에 집중해"*).
- **실행 로그와 세션:** 각 작업자 실행은 NDJSON 이벤트 스트림과 stderr를 실행별 로그 파일에 저장해 전체 출력이 메인 컨텍스트로 흘러들지 않게 합니다. 완료 후 메인 모델은 마지막 `result` 레코드(`subtype`, `sessionId`, `stopReason`, `usage`, `durationMs`, `finalText`)와 종료 코드를 확인하고, CLI가 실제로 반환한 `sessionId`와 로그 경로를 기록하며 CLI가 반환하기 전에 ID를 가정하지 않습니다. 같은 기능의 후속 수정은 컨텍스트가 유효한 동안 `cmd -p --resume <sessionId>`로 그 세션을 명시적으로 재개하고, 설계가 크게 바뀌었거나 컨텍스트가 오래됐으면 현재 상태를 정리한 간결한 핸드오프로 새 세션을 만듭니다. `usage`가 없으면 알 수 없음(unknown)으로 보고하고 비교 측정 없이 토큰 절감을 주장하지 않습니다.
- **작업자 인계:** 작업자는 상태, 변경 파일, 실제 검증 명령과 종료 코드·결과 요약, 증거 로그 경로, 미해결 항목을 보고합니다. 비용·시간·재작업은 실제로 측정된 경우에만 보고합니다.
